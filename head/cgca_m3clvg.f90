!$Id: cgca_m3clvg.f90 226 2016-02-19 12:37:29Z mexas $
...
!  CONTAINS
!    Submodules: m3clvg_sm1, m3clvg_sm2.
!    Abstract interfaces: cgca_clvgs_abstract.
!    Subroutines: cgca_clvgn, cgca_clvgsd, cgca_clvgsp, cgca_clvgp,
...
public :: cgca_clvgp_nocosum, cgca_clvgp1, cgca_clvgsd, cgca_clvgsp, &
          cgca_dacf, cgca_dacf1, cgca_tchk, cgca_gcupd, cgca_gcupdn, &
          cgca_clvgp

! This is a GC (grain connectivity) update coarray
! to store the (grain, neighbour) pairs from each image,
! up to a maximum of:
integer( kind=idef ), parameter :: gculen = 100_iarr
...
!
! The state must be set initially to intact.
! To easily distinuguish real records from blanks, it would
! be better to set grain numbers to some impossible values initially,
! e.g. negative. I might do this later.
!
integer( kind=iarr ) :: gcupd( gculen, 3 ) [*] = cgca_gb_state_intact

! This is a new gc update array. It uses multiple codimensions
! to make it easier to read data from neighbouring images
integer( kind=iarr ), allocatable :: gcupd_arr(:,:) [:,:,:]

! Counter of a gcupd_arr pair. Set to 1 initially.
...
interface
  module subroutine cgca_gcupd
  end subroutine cgca_gcupd
  ...
  module subroutine cgca_clvgp( coarray, rt, t, scrit, sub,            &
...
!    cgca_.
!    This routine allocates gcupd_arr array coarray. Therefore it
!    involves implicit sync all.
!    If gcupd_arr is allocated already,
!    it is first deallocated and then reallocated with new
!    codimensions.
!  SIDE EFFECTS
!    gcupd_arr is (re)allocated.
!  USES
!  USED BY
!  SOURCE
...
!    and add this GB fracture to gcupd_arr on this image.
!
!    Inputs:
  ...
  clv: if ( any( cgca_clvg_states .eq. farr(x1,x2,x3)) ) then
    ...
    same: if ( marr(centre,centre,centre) .eq. marr(x1,x2,x3) ) then
      ...
      crossgb: if ( intact .and.                                       &
        ...
        ! Add the (grain, neighbour) pair to gcupd_arr coarray
        ! on this image
        gcupd_arr( gcucnt, : ) = (/ marr( centre , centre , centre ) , &
                                marr(   x1   ,   x2   ,   x3   ) ,     &
                                cgca_gb_state_fractured /)
        ...
        if (debug)                                                     &
          write (*,"(4(a,i0),2(tr1,i0),')',a,27(i0,tr1),').')")        &
            ...
            ", gcucnt=", gcucnt,                                       &
            ", calling cgca_gcf, gcupd_arr=(", gcupd_arr( gcucnt , : ),&
            ", marr=(", marr

        ! increment the gcupd_arr pair counter
        gcucnt = gcucnt + 1

        ! Issue fatal error if the length of the gcupd_arr has been
        ! exceeded.
   ...
   inta: if ( intact ) then
    z2: if ( proj .lt. ltrshld ) then
     ...
     if (debug) write (*,"(2(a,i0),2(tr1,i0),').')")                   &
       ...
       ": called _gcf, set gcupd_arr=(", gcupd_arr
...
main: do
  ...
  ! Udate all local GC arrays using data from the nearest
  ! neigbouring images
