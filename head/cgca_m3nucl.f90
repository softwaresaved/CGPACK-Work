!$Id: cgca_m3nucl.f90 107 2015-06-12 16:30:57Z mexas $
...
if ( .not. allocated( coarray ) ) then
 write (*,'(a,i0)')                                                    &
     "ERROR: cgca_nr: coarray is not allocated, img: ", thisimage
 error stop 
end if

! check that there are only liqiud cells in coarray.
...
if ( any( coarray(lbr(1):ubr(1),lbr(2):ubr(2),lbr(3):ubr(3),           &
 ...
 write (*,'(a,i0)')                                                    &
  "ERROR: cgca_nr: non-liquid elements in coarray, img: ", thisimage
 error stop
end if
...
if ( number .lt. 1 ) then
 write (*,'(a,i0)')                                                    &
  "ERROR: cgca_nr: number of nuclei must be 1 or more, img:", thisimage
 error stop
end if
...
img1: if (image1) then
 ...
 if ( frac .gt. critfract )                                            &
   write (*,'(a,g10.3)')                                               &
    "WARN: cgca_nr: too many nuclei - no physical sense!&
    & nuclei/model size: ", frac
 ...
 do
  ...
  ! if a cell is not liquid then assign the running "nuc"
  ! number to it
  ncln: if (coarray(local(1),local(2),local(3),cgca_state_type_grain)  &
                [imgpos(1),imgpos(2),imgpos(3)] .le. 0) then
      coarray(local(1),local(2),local(3),cgca_state_type_grain)  &
              [imgpos(1),imgpos(2),imgpos(3)] = nuc
  
   ! if requested, dump some debug output
   if (debug) then
    write (*,'(a,"(",2(i0,","),i0,")")') "DEBUG: cgca_nr: imgpos ",imgpos
    write (*,'(a,"(",2(i0,","),i0,")")') "DEBUG: cgca_nr: local ",local
    write (*,'(a,i0)') "DEBUG: cgca_nr: nucleus ",nuc
   end if
