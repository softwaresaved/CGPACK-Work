!$Id: cgca_m3pfem.f90 173 2015-11-04 16:52:16Z mexas $
...
use :: cgca_m1co, only : iarr, idef, ilrg, ldef, rdef,                 &
  cgca_state_null, cgca_state_type_frac
...
images: do i=rndint, rndint+nimgs-1
  ...
  allocate( tmp( ndims, nelements ), source=0.0_cgca_pfem_iwp,         &
            stat=errstat )
  if ( errstat .ne. 0 )                                                &
    error stop "ERROR: cgca_pfem_cenc: allocate( tmp )"
...
integer( kind=idef ), intent( in ) :: lc(3)
...
main: do i = 1, 8
  ...
  call cgca_pfem_cellin( local( i, : ), lres, bcol, charlen, debug,    &
    flagarr(i) )
