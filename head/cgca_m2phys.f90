!$Id: cgca_m2phys.f90 121 2015-10-02 13:19:16Z mexas $
...
integer( kind=idef ), intent( inout ) :: ir(size( bsz ))

!  OUTPUT

integer( kind=iarr ), intent( out ) :: c(size( bsz ))
real( kind=rdef ), intent( out ) :: lres
integer( kind=idef ), intent( out ) :: ng
...
real( kind=rdef ) :: &
 ...
 dm3                   ! dm**3
integer( kind=idef ) :: irrank(3), tmp

! sort box directions: 1, 2, and 3, in decreasing order
! of dimension, i.e. direction 1 is where the dimension
! is biggest, direction 3 is where the dimension is smallest.
! We start assuming d(1) >= d(2) >= d(3). If this is so
! then no rearrangement is required, and no IF statements
! will be true.

! irrank contains the directions of the box, ranked from
! the biggest to the smallest.
irrank = (/ 1, 2, 3 /)

if ( bsz(1) .lt. bsz(2) ) then
        tmp = irrank(1)
  irrank(1) = irrank(2)
  irrank(2) = tmp
end if

if ( bsz(2) .lt. bsz(3) ) then
        tmp = irrank(2)
  irrank(2) = irrank(3)
  irrank(3) = tmp
end if
...
! irrank now contains sorted indices of box dimension
! array. So use it as index to rearrange ir.
ir = ir(irrank)
