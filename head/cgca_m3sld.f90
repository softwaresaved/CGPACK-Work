!$Id: cgca_m3sld.f90 224 2016-02-19 09:14:09Z mexas $
...
logical( kind=ldef ), save ::                                          &
  ...
  allfinished [*]    ! true if all coarray values are positive
                     ! on all images
...
if ( thisimage .eq. 1 ) then
  ...
  if ( .not. allyesnuc ) then
    write (*,'(a)') "ERROR: cgca_sld: no nuclei found"
    error stop
  end if

  ! report an error if no liquid phase found
  if ( allfinished ) then
    write (*,'(a)') "ERROR: cgca_sld: already solid"
    error stop
  end if
...
if ( errstat .ne. 0 ) then
  write (*,'(a,i0)')                                                   &
    "ERROR: cgca_sld: allocate( array ), img: ", thisimage
  error stop
end if
...
finished = .true.
if ( any( coarray( lbr(1):ubr(1) , lbr(2):ubr(2) , lbr(3):ubr(3),      &
   cgca_state_type_grain ) .eq. cgca_liquid_state ) ) finished = .false.
...
if ( errstat .ne. 0 ) then
  write (*,'(a,i0)')                                                   &
    "ERROR: cgca_sld: deallocate( array ), img: ", thisimage
  error stop
end if
