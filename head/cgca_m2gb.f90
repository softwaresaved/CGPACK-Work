!$Id: cgca_m2gb.f90 210 2016-02-17 16:31:00Z mexas $
...
!    cgca_agc (private to this module)
!  USES
!    cgca_m1co
!  USED BY
!    cgca_m3clvg
...
integer( kind=iarr ) :: state(26,2) = 0_iarr, i, j, gclen
...
integer( kind=idef ), intent(in) :: len
...
integer( kind=iarr ) :: i, tmp, grain1, grain2, match1, gclen
integer :: matches, img
...
integer( kind=iarr ) :: i, tmp, grain1, grain2
integer :: img
logical :: match
...
if (errstat .ne. 0) then
  write (*,'(2(a,i0))') &
    "ERROR: cgca_gbs: image: ", this_image(), &
...
do x1 = lbr(1), ubr(1)
  ...
  do i=1,27
    neigh(i,2) = count( neigh(:,1) .eq. neigh(i,1) )
  end do
...
if (errstat .ne. 0) then
  write (*,'(2(a,i0))') &
    "ERROR: cgca_gbs: image: ", this_image(), &
    ": cannot deallocate array, error code=", errstat
  error stop
end if
