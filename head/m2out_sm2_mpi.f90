!$Id: m2out_sm2_mpi.f90 229 2016-02-19 15:57:12Z mexas $
...
if ( product( coarsize ) .ne. nimgs) then
 write(*,*) 'ERROR: cgca_pswci: non-conforming coarray'
 error stop
end if
...
if ( mpisize .ne. nimgs .or. rank .ne. img-1 ) then
  write(*,*) 'ERROR: cgca_pswci: MPI/coarray mismatch'
  error stop
end if
...
if ( ierr .ne. MPI_SUCCESS ) then
 write (*,*)                                                           &
  'ERROR: cgca_pswci: MPI_type_create_subarray filetype: rank ', rank
 error stop
end if
...
if ( ierr .ne. MPI_SUCCESS ) then
 write (*,*)                                                           &
  'ERROR: cgca_pswci: MPI_type_create_subarray mpi_subarray: rank ',   &
    rank
 error stop
end if
...
rm: if ( img .eq. 1 ) then
  ...
  if ( ierr .ne. 0 ) then
    write (*,*) "ERROR: cgca_m2mpiio: open( fname ), iostat=",ierr
    error stop
  end if

  write( funit, * , iostat=ierr) ""
  if ( ierr .ne. 0 ) then
    write (*,*) "ERROR: cgca_m2mpiio: write( fname ), iostat=",ierr
    error stop
  end if
...
!  end if
  ...
  if ( ierr .ne. 0 ) then
    write (*,*) "ERROR: cgca_m2mpiio: close( fname ), iostat=",ierr
    error stop
  end if
...
if ( ierr .ne. MPI_SUCCESS ) then
  write (*,*) 'ERROR: cgca_pswci: MPI_file_set_view: rank ', rank
  error stop
end if
...
call MPI_File_write_all( fh, coarray(:,:,:,stype), 1, mpi_subarray,    &
                         mpistat, ierr )
if ( ierr .ne. MPI_SUCCESS ) then
  write (*,*) 'ERROR: cgca_pswci: MPI_file_write_all: rank ', rank
  error stop
end if
...
if ( ierr .ne. MPI_SUCCESS ) then
 write (*,*) 'ERROR: cgca_pswci: MPI_file_close: rank ', rank
 error stop
end if
