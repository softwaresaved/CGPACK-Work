!$Id: cgca_m2rot.f90 144 2015-10-21 16:40:36Z mexas $
...
!  CONTAINS
!    cgca_rt, cgca_ckrt, rtprint, cgca_csym, cgca_mis, cgca_miscsym
!  USES
!    cgca_m1co
!  USED BY
!    cgca
...
private
public :: cgca_ckrt, cgca_csym, cgca_mis, cgca_miscsym, cgca_rt
...
! number. dimensions 2 and 3 are for the rotation tensor for the grain,
! e.g. r(387,2,1) is the rotation tensor component r21 for grain 387.
!
! image 1 assigns rotation tensors to all grains. then all other
! images copy the coarray from image 1.
!
! the method: choose 3 random angles. then interpret them as rotations
! about axes 1, 2 and 3 in turn. the resulting rotation will be quite
! random.
!
...
flag = 0 
 ...
 do i=1,2
   if (i.eq.1) orthogon = matmul(r,transpose(r))
   if (i.eq.2) orthogon = matmul(transpose(r),r)

   if (abs(orthogon(1,1)-one) .gt. maxerr) then
     if (debug) call rtprint(r,transpose(r),orthogon,maxerr)
     flag=1*i
     return
   end if
